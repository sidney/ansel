...
void gui_init(dt_lib_module_t *self)
{
  ...
  /* basic */
  int g_index = DT_MODULEGROUP_BASIC;
  ...
  /* correct */
  g_index = DT_MODULEGROUP_CORRECT;
...
static void _lib_modulegroups_update_iop_visibility(dt_lib_module_t *self)
{
  ...
  if(modules)
  {
    ...
    do
    {
      ...
      if(FALSE) // (module->iop_order == INT_MAX)
      {
...
static void _lib_modulegroups_toggle(GtkWidget *button, gpointer user_data)
{
  ...
  if(d->current == cb && !(text_entered && text_entered[0] != '\0'))
