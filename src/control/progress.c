...
static void global_progress_start(dt_control_t *control, dt_progress_t *progress)
{
  ...
  progress->darktable_launcher = unity_launcher_entry_get_for_desktop_id("ansel.desktop");
  ...
  if(darktable.dbus && darktable.dbus->dbus_connection)
  {
    ...
    GVariant *params = g_variant_new("(sa{sv})", "application://ansel.desktop", &builder);
...
void dt_control_progress_init(struct dt_control_t *control)
{
  ...
  UnityLauncherEntry *darktable_launcher = unity_launcher_entry_get_for_desktop_id("ansel.desktop");
